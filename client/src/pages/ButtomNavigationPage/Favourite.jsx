...
import { getFavoriteFunction } from "../../context/MenuFunction";
import FileItem from "../../component/FileItem";
import FileViewerModal from "../../components/FileViewerModal";
import "../../styles/ButtomNavigationStyle/Favorite.css";
  ...
  useEffect(() => {
    const fetchFolderContents = async () => {
      try {
        ...
        const apiCall = getFavoriteFunction();
        const response = await apiCall;
        if (response.success) {
          setFavorite(response.message);
        } else {
    ...
    const handleAction = async () => {
      try {
        if (actionData.actionType === "delete") {
          ...
          setFavorite((prevFiles) => [result, ...prevFiles]);
          setRefreshTrigger(true);
        }
      } catch (error) {
